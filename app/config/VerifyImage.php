...
class VerifyImage
{
     ...
     * @param int $code_length
     */
    public function CreateVerifyImage($width = 80, $height = 35, $pixed_num = 80, $line_num = 5, $code_type = 3, $code_length = 4) {
        $this->m_verify_code = $this->CreateRandomVerifyCode($code_type, $code_length);
        $m_image = imagecreatetruecolor($width, $height);
        $white = imagecolorallocate($m_image, 255, 255, 255);
        imagefill($m_image, 0, 0, $white);
//        注意：将字体导入到fonts目录下
        $font_files = array('./fonts/arial.ttf');
...
//            将字符串显示到一个居中的位置
            ...
            $text = substr($this->m_verify_code, $i, 1);
